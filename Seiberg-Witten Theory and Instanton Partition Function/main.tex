...
Inspired by the Montonen-Olive conjecture in the four-dimensional $\mathcal{N}=4$ theory, Seberg and Witten found that there is an ``electro-magnetic duality'' in the theory, and the coupling constant $g$ of the original theory becomes $1/g$ under this duality, so the infrared strong coupling region of the original theory becomes infrared-free in the dual theory, which can be solved by perturbation theory, so the infrared vacuum structure can be easily determined.

Then they obtained the contribution of the instanton by using the monodromy method in the vacuum moduli space. And the prepotential of the theory was derived, which means the low-energy effective theory was completely solved.

Nekrasov found a method to obtain the prepotential through direct calculation, that is, the method of ``instanton counting''. By using the supersymmetric localization, he turns the calculation of the prepotential into solving a supersymmetric quantum mechanics on a specific background and finally into the computing the residue through summation over Young tableaux to obtain the so-called instanton partition function. The prepotential can be obtained by taking the logarithm of the instanton partition function.

For real-world quantum chromodynamics (QCD), since it is asymptotically free, little is known about its low-energy effective theory and phenomena such as quark confinement. In the Seiberg-Witten theory, magnetic monopoles are massless degrees of freedom in certain vacuum states producing so-called ``monopole condensations''. Analogous to the ``Cooper pair'' in superconductors, monopole condensation also produces a phenomenon similar to the Meissner effect, that is, the superconductor completely excludes the magnetic field; while for QCD, the ``magnetic field'' becomes a ``color magnetic field'', which leads to what is more generally known as the phenomenon of quark confinement. At the same time, monopole condensation also gives rise to the breaking of chiral symmetry. When analyzing non-supersymmetric QCDs, 't Hooft proposed the hypothesis that monopole condensation will cause quark confinement, but they were unable to perform actual calculations. However, the Seiberg-Witten theory gives an excellent example . This is undoubtedly a deeper understanding of the confinement problem in QCD.

As can be seen from the above cases, it is relatively easy to analyze supersymmetric QCDs because of their stronger symmetry. From this, we can get a first glimpse of the mysterious non-perturbative phenomena in quantum field theory, and then have more insights into non-supersymmetric QCDs. 
